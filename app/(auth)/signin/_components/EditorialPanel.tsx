  ...
  return (
    <aside className="relative flex flex-col overflow-hidden bg-[var(--ink)] px-7 pb-10 pt-7 text-[var(--paper)] md:px-10">
      <div
        ...
        style={{
          background:
            "radial-gradient(circle, rgb(62 106 140 / 24%) 0%, transparent 60%)",
        }}
      />
      <div
        aria-hidden="true"
        ...
        style={{
          background:
            "radial-gradient(circle, rgb(185 58 29 / 18%) 0%, transparent 60%)",
        }}
      />
      ...
      {floatingStats.map((stat) => (
        <div
          key={stat.label}
          aria-hidden="true"
          className={`pointer-events-none absolute z-[3] hidden rounded-[10px] border border-[var(--rule-dark)] bg-[rgb(246_241_230_/_8%)] px-3.5 py-2.5 backdrop-blur-md xl:block ${
            stat.position === "a"
      ...
      <div className="relative z-[2] mx-auto flex w-full max-w-[500px] flex-1 flex-col justify-center">
        ...
        <blockquote className="font-display mb-5 text-[38px] leading-[1.08] font-[320] italic tracking-[-0.025em] text-[var(--paper)] max-[1100px]:text-[32px] max-sm:text-[28px]">
          ...
          {customerQuote.body}{" "}
          <em className="italic text-[var(--accent-light)]">{customerQuote.emphasis}</em>
          <span className="not-italic text-[var(--accent-light)]">”</span>
        </blockquote>
        ...
        <div className="mb-12 flex items-center gap-3">
          <div
            ...
            style={{
              background: "linear-gradient(135deg, var(--sky), var(--moss))",
            }}
          >
            {customerQuote.initials}
          </div>
          <div>
        ...
        <DashboardPreview greeting="Welcome back," name="Marcus" kpis={dashKpis} />
          ...
          {changelogItems.map((item) => (
            <div key={item.title} className="flex items-center gap-3 text-sm text-[rgb(246_241_230_/_90%)]">
              <span className="grid size-[22px] flex-none place-items-center rounded-full bg-[rgb(61_90_58_/_20%)] text-[#8FB58B]">
                <CheckBoldIcon className="h-3 w-3" />
              </span>
              <span>

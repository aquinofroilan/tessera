...
type Bucket = {
    label: string;
    key: keyof ApAgingSummary["counts"];
...
const buckets: Bucket[] = [
    ...
    { label: "Total owed", key: "total", valueKey: "total", tone: "ink" },
];
...
export function AgingStrip({ summary, currencyCode }: { summary: ApAgingSummary; currencyCode: string }) {
    return (
        <section aria-label="Accounts payable aging" className="grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-6">
            {buckets.map((b) => {

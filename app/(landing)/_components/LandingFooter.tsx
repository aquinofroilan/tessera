...
export function LandingFooter() {
  return (
    <footer className="border-t border-[var(--rule)] px-0 pb-9 pt-12 text-[var(--ink-soft)]">
      <div className="mx-auto w-full max-w-[1240px] px-7">
        <div className="mb-12 grid gap-10 sm:grid-cols-2 lg:grid-cols-[1.5fr_repeat(4,minmax(0,1fr))]">
          <div>
            <a
              href="#"
              className="font-display mb-3 inline-flex items-baseline gap-0.5 text-[30px] italic tracking-[-0.02em] text-[var(--ink)]"
            >
              Loom
              <span className="mb-[3px] ml-0.5 size-1.5 self-center rounded-full bg-[var(--accent)]" />
            </a>
            <p className="max-w-[28ch] text-sm">
              Software for running the whole shop. Made in Portland, Oregon. Serving operators everywhere since 2019.
            </p>
          </div>

          {footerColumns.map((column) => (
            <div key={column.heading}>
              <h4 className="mb-3.5 font-mono text-[11px] uppercase tracking-[0.12em] text-[var(--muted)]">
                {column.heading}
              </h4>

        ...
        <aside
            ...
            {floatingStats.slice(0, 2).map((stat, i) => (
                <div
                    ...
                    <div className="font-display mt-0.5 text-lg font-normal tracking-[-0.02em] text-[var(--paper)] tabular-nums">
                        {stat.value}
                    </div>
                </div>
            ))}
            ...
            <header className="relative z-[2] mb-10 flex justify-end">
                <div className="inline-flex items-center gap-2 rounded-full border border-[var(--rule-dark)] px-3 py-1.5 font-mono text-[11px] tracking-[0.08em] text-[rgb(246_241_230_/_70%)] uppercase">
                    <span className="size-1.5 animate-[pulse_2.4s_ease-in-out_infinite] rounded-full bg-[var(--moss)] shadow-[0_0_0_3px_rgb(61_90_58_/_30%)]" />
                    {statusChip}
                </div>
            </header>

            <div className="relative z-[2] mx-auto flex w-full max-w-125 flex-1 flex-col justify-center">
                <div className="mb-7 flex items-center gap-2.5 font-mono text-[11px] tracking-[0.18em] text-[var(--accent-light)] uppercase before:h-px before:w-6 before:bg-[var(--accent-light)] before:content-['']">
                    {kicker}
                </div>

                <blockquote className="font-display mb-5 text-[38px] leading-[1.08] font-[320] tracking-[-0.025em] text-[var(--paper)] italic max-[1100px]:text-[32px] max-sm:text-[28px]">
                    <span className="text-[var(--accent-light)] not-italic">“</span>
                    {quote.body} <em className="text-[var(--accent-light)] italic">{quote.emphasis}</em>
                    <span className="text-[var(--accent-light)] not-italic">”</span>
                </blockquote>

                <div className="mb-12 flex items-center gap-3">
                    <div
                        className="font-display grid size-9.5 place-items-center rounded-full text-[15px] font-medium text-[var(--paper)] shadow-[inset_0_0_0_2px_rgb(255_255_255_/_20%)]"
                ...
                <div className="grid gap-3">
                    <div className="mb-1.5 border-b border-[var(--rule-dark)] pb-2.5 font-mono text-[10px] tracking-[0.16em] text-[rgb(246_241_230_/_50%)] uppercase">
                        {list.label}
                    </div>
                    {list.items.map((item) => (
